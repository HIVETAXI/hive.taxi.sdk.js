...
describe 'HiveTaxi.Model.Shape', ->

  describe 'Traits', ->
    it 'inherits streaming trait', ->
      api = new HiveTaxi.Model.Api shapes: S1: { 'type': 'blob', 'streaming': true }
      shape = HiveTaxi.Model.Shape.create { members: { body: { shape: 'S1' } } }, { api: api }
      expect(shape.members.body.isStreaming).to.eql(true)

  describe 'TimestampShape', ->
    describe 'toType()', ->
      it 'converts unix timestamps', ->
        api = new HiveTaxi.Model.Api metadata: timestampFormat: 'unixTimestamp'
        shape = HiveTaxi.Model.Shape.create { type: 'timestamp' }, { api: api }
        date = shape.toType(12300)
        expect(date).to.eql(new Date(12300000))

      it 'converts iso8601 timestamps', ->
        api = new HiveTaxi.Model.Api metadata: timestampFormat: 'iso8601'
        shape = HiveTaxi.Model.Shape.create { type: 'timestamp' }, { api: api }
        date = shape.toType('1970-01-01T00:00:00Z')
        expect(date).to.eql(new Date(0))

      it 'converts rfc822 timestamps', ->
        api = new HiveTaxi.Model.Api metadata: timestampFormat: 'rfc822'
        shape = HiveTaxi.Model.Shape.create { type: 'timestamp' }, { api: api }
    ...
    describe 'toWireFormat()', ->
      it 'converts all header shapes to rfc822', ->
        api = new HiveTaxi.Model.Api metadata: timestampFormat: 'unixTimestamp'
        shape = HiveTaxi.Model.Shape.create { type: 'timestamp', location: 'header' }, { api: api }
      ...
      it 'converts all timestamps in JSON protocol to unixTimestamp', ->
        # json rpc
        api = new HiveTaxi.Model.Api metadata: protocol: 'json'
        shape = HiveTaxi.Model.Shape.create { type: 'timestamp' }, { api: api }
        date = shape.toWireFormat(new Date(12300000))
        expect(date).to.equal(12300)

        # rest-json
        api = new HiveTaxi.Model.Api metadata: protocol: 'rest-json'
        shape = HiveTaxi.Model.Shape.create { type: 'timestamp' }, { api: api }
        date = shape.toWireFormat(new Date(12300000))
        expect(date).to.equal(12300)
      ...
      it 'converts all timestamps in XML/query protocol to iso8601', ->
        # rest-xml
        api = new HiveTaxi.Model.Api metadata: protocol: 'rest-xml'
        shape = HiveTaxi.Model.Shape.create { type: 'timestamp' }, { api: api }
        date = shape.toWireFormat(new Date(12300000))
        expect(date).to.equal('1970-01-01T03:25:00Z')

        # query
        api = new HiveTaxi.Model.Api metadata: protocol: 'query'
        shape = HiveTaxi.Model.Shape.create { type: 'timestamp' }, { api: api }
        date = shape.toWireFormat(new Date(12300000))
        expect(date).to.equal('1970-01-01T03:25:00Z')
    ...
    describe 'BooleanShape', ->
      describe 'toType()', ->
        shape = HiveTaxi.Model.Shape.create { type: 'boolean' }
