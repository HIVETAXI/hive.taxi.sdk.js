...
make = (obj, opts) -> new HiveTaxi.Model.Api(obj, opts)

describe 'HiveTaxi.Model.Api', ->
  describe 'loading', ->
    it 'loads properties from metadata', ->
      api = make metadata:
        apiVersion: '1.0', endpointPrefix: 'endpoint', globalEndpoint: 'global'
        signatureVersion: 'v1', protocol: 'json', timestampFormat: 'rfc822'
      ...
      expect(api.signatureVersion).to.equal('v1')
  ...
  describe 'className', ->
    it 'generates the correct class name from fullName', ->
      api = make metadata: serviceFullName: 'HiveTaxi Service Name HeRE'
      expect(api.className).to.equal('ServiceNameHeRE')

    it 'uses abbreviation if supplied', ->
      api = make metadata: serviceAbbreviation: 'HiveTaxi SNH'
