...
Operation = helpers.HiveTaxi.Model.Operation
Api = helpers.HiveTaxi.Model.Api

svc = HiveTaxi.Protocol.Rest
describe 'HiveTaxi.Protocol.Rest', ->

  MockRESTService = helpers.util.inherit HiveTaxi.Service,
  ...
  beforeEach ->
    ...
    HiveTaxi.Service.defineMethods(MockRESTService)
    ...
    describe 'headers', ->
      beforeEach ->
        input =
          type: 'structure'
          members:
            ACL: location: 'header', locationName: 'x-hive-acl'

      it 'populates the headers with present params', ->
        request.params = ACL: 'public-read'
        defop input: input
        expect(build().httpRequest.headers['x-hive-acl']).to.equal('public-read')
      ...
      it 'works with map types', ->
        ...
        build()
        expect(request.httpRequest.headers['x-hive-meta-foo']).to.equal('bar')
  ...
  describe 'extractData', ->
    ...
    describe 'headers', ->
      ...
      it 'extracts map types from header', ->
        output.members.Metadata = type: 'map', location: 'headers', locationName: 'x-hive-meta-'
        response.httpResponse.headers['X-HIVE-META-FOO'] = 'foo'
        response.httpResponse.headers['x-hive-meta-bar'] = 'bar'
