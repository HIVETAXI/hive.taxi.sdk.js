...
svc = HiveTaxi.Protocol.Json
describe 'HiveTaxi.Protocol.Json', ->

  MockJSONClient = HiveTaxi.util.inherit HiveTaxi.Service,
    endpointPrefix: 'mockservice'
    api: new HiveTaxi.Model.Api
      metadata:
        protocol: 'json'
        targetPrefix: 'prefix'
      operations:
  ...
  HiveTaxi.Service.defineMethods(MockJSONClient)
  ...
  beforeEach ->
    ...
    request = new HiveTaxi.Request(service, 'operationName')
  ...
  describe 'buildRequest', ->
    ...
    it 'should set Content-Type header', ->
      ...
      expect(request.httpRequest.headers['Content-Type']).
        to.equal('application/x-hive-json-1.0')

    it 'should set X-Hive-Target header', ->
      buildRequest()
      expect(request.httpRequest.headers['X-Hive-Target']).
    ...
    it 'should set the body to JSON serialized params', ->
      service.api.operations.operationName.input.members.foo =
        new HiveTaxi.Model.Shape.create({type: 'string'}, api: service.api)
    ...
    it 'should preserve numeric types', ->
      service.api.operations.operationName.input.members.count =
        new HiveTaxi.Model.Shape.create({type: 'integer'}, api: service.api)
  ...
  describe 'extractError', ->
    ...
    it 'removes prefixes from the error code', ->
      extractError '{"__type":"com.hivetaxi.coral.service#ErrorCode" }'
