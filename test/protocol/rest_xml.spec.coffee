...
svc = HiveTaxi.Protocol.RestXml
describe 'HiveTaxi.Protocol.RestXml', ->

  MockRESTXMLService = HiveTaxi.util.inherit HiveTaxi.Service,
  ...
  beforeEach ->
    MockRESTXMLService.prototype.api = new HiveTaxi.Model.Api
      metadata:
        xmlNamespace: xmlns
      operations:
        SampleOperation:
    ...
    HiveTaxi.Service.defineMethods(MockRESTXMLService)
    service = new MockRESTXMLService(region: 'region')
    request = new HiveTaxi.Request(service, 'sampleOperation')
    response = request.response

  defop = (op) ->
    HiveTaxi.util.property(service.api.operations, 'sampleOperation',
      new HiveTaxi.Model.Operation('sampleOperation', op, api: service.api))
  ...
  describe 'buildRequest', ->
          ...
          input:
            ...
            members:
              Bucket:
                location: 'uri'
              ACL:
                locationName: 'x-hive-acl'
                location: 'header'

        build()
        expect(request.httpRequest.body).to.equal('')
        expect(request.httpRequest.path).to.equal('/abc')
        expect(request.httpRequest.headers['x-hive-acl']).to.equal('canned-acl')
    ...
    describe 'xml bodies', ->
      it 'populates the body with XML from the params', ->
        ...
        expect(request.httpRequest.headers['x-hive-acl']).to.equal('canned-acl')
        expect(request.httpRequest.headers['x-hive-meta-abc']).to.equal('xyz')
        expect(request.httpRequest.headers['x-hive-meta-mno']).to.equal('hjk')
  ...
  describe 'extractData', ->
    ...
    it 'sets payload element along with other outputs', ->
      response.httpResponse.headers['x-hive-foo'] = 'foo'
      response.httpResponse.headers['x-hive-bar'] = 'bar'
      defop output:
        type: 'structure'
        payload: 'Baz'
        members:
          Foo:
            location: 'header'
            locationName: 'x-hive-foo'
          Bar:
            location: 'header'
            locationName: 'x-hive-bar'
          Baz: {}
      extractData 'Buffer data'
