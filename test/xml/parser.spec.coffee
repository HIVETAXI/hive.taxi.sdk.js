...
describe 'HiveTaxi.XML.Parser', ->

  parse = (xml, rules, callback) ->
    if rules
      shape = HiveTaxi.Model.Shape.create(rules, api: protocol: 'rest-xml')
    else
      shape = {}
    callback.call(this, new HiveTaxi.XML.Parser().parse(xml, shape))
              ...
              members:
                ...
                Attr2:
                  ...
                  members:
                    Foo: {}
      xml = """
      <xml xmlns="http://sdk.hivetaxi.com/doc/1.0/">
        <List>
          <member>
  ...
  describe 'parsing errors', ->
    ...
    it 'throws an error when unable to parse the xml', ->
      ...
      error = {}
      try
        new HiveTaxi.XML.Parser().parse(xml, rules)
      catch e
        error = e
