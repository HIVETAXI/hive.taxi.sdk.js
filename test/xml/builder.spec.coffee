...
describe 'HiveTaxi.XML.Builder', ->

  xmlns = 'http://mockservice.com/xmlns'
  api = null

  beforeEach ->
    api = new HiveTaxi.Model.Api metadata: xmlNamespace: xmlns

  toXML = (rules, params) ->
    rules.type = 'structure'
    shape = HiveTaxi.Model.Shape.create(rules, api: api)

    builder = new HiveTaxi.XML.Builder()
  ...
  describe 'timestamps', ->
    ...
    it 'iso8601', ->
      ...
      xml = """
      <Data xmlns="#{xmlns}">
        <Expires>#{HiveTaxi.util.date.iso8601(time)}</Expires>
      </Data>
      """
      matchXML(toXML(rules, params), xml)
    ...
    it 'rfc822', ->
      ...
      xml = """
      <Data xmlns="#{xmlns}">
        <Expires>#{HiveTaxi.util.date.rfc822(time)}</Expires>
      </Data>
      """
      matchXML(toXML(rules, params), xml)
    ...
    it 'unix timestamp', ->
      ...
      xml = """
      <Data xmlns="#{xmlns}">
        <Expires>#{HiveTaxi.util.date.unixTimestamp(time)}</Expires>
      </Data>
      """
      matchXML(toXML(rules, params), xml)
