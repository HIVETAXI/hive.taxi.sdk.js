...
describe 'HiveTaxi.HttpRequest', ->

  request = null
  agentHeader = null
  if HiveTaxi.util.isBrowser()
    agentHeader = 'X-Hive-User-Agent'
  else
    agentHeader = 'User-Agent'

  beforeEach ->
    request = new HiveTaxi.HttpRequest('http://domain.com')
  ...
  describe 'constructor', ->
    ...
    it 'provides headers with a default user agent', ->
      headers = {}
      headers[agentHeader] = HiveTaxi.util.userAgent()
      expect(request.headers).to.eql(headers)

    it 'adds the customUserAgent to the user agent header if provided as string', ->
      headers = {}
      headers[agentHeader] = HiveTaxi.util.userAgent() + ' custom'
      request = new HiveTaxi.HttpRequest('http://domain.com', '', 'custom')
      expect(request.headers).to.eql(headers)
    ...
    it 'uses the path from the endpoint if provided', ->
      request = new HiveTaxi.HttpRequest('http://domain.com/path')
