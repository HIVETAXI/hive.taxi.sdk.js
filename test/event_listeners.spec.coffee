...
describe 'HiveTaxi.EventListeners', ->
  ...
  beforeEach (done) ->
    ...
    service.config.credentials = HiveTaxi.util.copy(service.config.credentials)
  ...
  describe 'validate', ->
    ...
    it 'sends error event if credentials are not set', ->
      ...
      HiveTaxi.util.arrayEach errorHandler.calls, (call) ->
    ...
    it 'does not validate credentials if request is not signed', ->
      ...
      service.api = new HiveTaxi.Model.Api metadata:
  ...
  describe 'afterBuild', ->
    ...
    describe 'adds Content-Length header', ->
      ...
      it 'builds Content-Length for buffer body', ->
        expect(contentLength(new HiveTaxi.util.Buffer('tï№'))).to.equal(6)
  ...
  describe 'sign', ->
    ...
    it 'uses the api.signingName if provided', ->
      ...
      helpers.spyOn(HiveTaxi.Signers.RequestSigner, 'getVersion').andCallFake ->
  ...
  describe 'send', ->
    it 'passes httpOptions from config', ->
      options = {}
      helpers.spyOn(HiveTaxi.HttpClient, 'getInstance').andReturn handleRequest: (req, opts) ->
        options = opts
        new HiveTaxi.SequentialExecutor()
  ...
  describe 'httpHeaders', ->
    ...
    it 'applies clock skew offset when correcClockSkew is true', ->
      ...
      helpers.spyOn(HiveTaxi.util, 'isClockSkewed').andReturn true
      request = makeRequest()
      response = request.send()
      offset = Math.abs(HiveTaxi.config.systemClockOffset)
  ...
  if HiveTaxi.util.isNode() and HiveTaxi.HttpClient.streamsApiVersion > 1
  ...
  describe 'retry', ->
    ...
    ['ExpiredToken', 'ExpiredTokenException', 'RequestExpired'].forEach (name) ->
      it 'invalidates expired credentials and retries', ->
        helpers.spyOn(HiveTaxi.HttpClient, 'getInstance')
        HiveTaxi.HttpClient.getInstance.andReturn handleRequest: (req, opts, cb, errCb) ->
          if req.headers.Authorization == 'Basic SU5WQUxJREtFWTpJTlZBTElEU0VDUkVU'
        ...
        service.config.signatureVersion = 'basic_auth'
        response = makeRequest(->)
    ...
    ['RequestTimeTooSkewed', 'RequestExpired', 'RequestInTheFuture',
      ...
      it 'retries clock skew errors', ->
        ...
        HiveTaxi.config.isClockSkewed = true
        service = new MockService({maxRetries: 3, correctClockSkew: true})
        request = makeRequest()
        request.on 'extractError', (resp) ->
          resp.error =
  ...
  describe 'logging', ->
    ...
    logger = null
    logfn = (d) -> data += d
    match = /\[HiveTaxi mock 200 .* 0 retries\] mockMethod\(.*foo.*bar.*\)/
