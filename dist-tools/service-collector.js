...
var serviceClasses = {};
Object.keys(HiveTaxi).forEach(function(name) {
  if (HiveTaxi[name].serviceIdentifier) {
    serviceClasses[HiveTaxi[name].serviceIdentifier] = HiveTaxi[name];
  }
});
...
function getServiceHeader(service) {
  ...
  var file = util.format(
    'if (!Object.prototype.hasOwnProperty.call(HiveTaxi, \'' + metadata[service].name + '\')) {\n' +
    '  HiveTaxi.apiLoader.services[\'%s\'] = {};\n' +
    '  HiveTaxi.%s = HiveTaxi.Service.defineService(\'%s\', %s);\n',
...
function getService(service, version) {
  ...
  var line = util.format(
    'HiveTaxi.apiLoader.services[\'%s\'][\'%s\'] = %s;\n',
  ...
  if (Object.prototype.hasOwnProperty.call(api, 'paginators') && Object.keys(api.paginators).length) {
    line = util.format(
      'HiveTaxi.apiLoader.services[\'%s\'][\'%s\'].paginators = %s;\n',
  ...
  if (Object.prototype.hasOwnProperty.call(api, 'waiters') && Object.keys(api.waiters).length) {
    line = util.format(
      'HiveTaxi.apiLoader.services[\'%s\'][\'%s\'].waiters = %s;\n',
