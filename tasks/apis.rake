  ...
  namespace :api do
    ...
    namespace(service) do
      ...
      task(:'version:html') do
        ...
        puts "  <td>#{version}</td>\n  <td>HiveTaxi.#{klass}</td>\n</tr>"
      end

      task(:'version:md') do
        base = "http://docs.hivetaxi.com/JavaScriptSDK/latest"
        puts "* [HiveTaxi.#{klass}](#{base}/#{klass}.html)"
      end
    end
...
namespace :api do
  task :'versions:html' => :'api:versions:internal' do
    ...
    $apis.sort_by{|svc,apis| svc.downcase }.each do |(svc,apis)|
      ...
      rows <<   "<td#{rowspan}>HiveTaxi.#{apis.first.last}</td>" if ENV['README']
